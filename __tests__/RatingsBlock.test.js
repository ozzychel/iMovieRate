...
describe('<RatingsBlock> component testing', () => {
  ...
  it('renders correct imdb logo', () => {
    let elem = testInstance.findByProps({testID:'imdb_logo'});
    expect(elem.props.source.uri).toBe('https://ia.media-imdb.com/images/M/MV5BODc4MTA3NjkzNl5BMl5BcG5nXkFtZTgwMDg0MzQ2OTE@._V1_.png');
  });

  it('renders correct imdb rating', () => {
    let elem = testInstance.findByProps({testID:'imdb_rating'});
    expect(elem.props.children).toBe('8.2 / 10');
  });

  it('renders N/A if no imbd rating provided', () => {
    testRenderer = create(<RatingsBlock
      movie_omdb={deleteRating(mockData.omdb[0], 'imdbRating')}/>);
    testInstance = testRenderer.root;
    let elem = testInstance.findByProps({testID:'imdb_rating'});
  ...
  it('renders correct metascore rating', () => {
    let elem = testInstance.findByProps({testID:'mscore_rating'});
    expect(elem.props.children).toBe('70');
  });

  it('renders N/A if no metascore rating provided', () => {
    testRenderer = create(<RatingsBlock
      movie_omdb={deleteRating(mockData.omdb[0], 'Metascore')}/>);
    testInstance = testRenderer.root;
    let elem = testInstance.findByProps({testID:'mscore_rating'});
    expect(elem.props.children).toBe('N/A');
  });
